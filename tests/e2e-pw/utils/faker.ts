...
/**
 * Create a Lead through the admin UI.
 *
 * Ported from the Admin package's lead.spec.ts `generateLead` flow so this
 * suite stays self-contained. The person lookup uses v-model.lazy +
 * v-debounce, so the search input needs a manual 'change' dispatch plus a
 * short wait before "Add as New" is offered.
 *
 * @param {import('@playwright/test').Page} page  An authenticated admin page.
 * @returns {Promise<{ leadTitle: string, leadEmail: string }>}
 */
async function createLead(page) {
    ...
    await page.fill('textarea[name="description"]', generateDescription());
    await page.locator('select[name="lead_source_id"]').selectOption('1');
    await page.fill('input[name="expected_close_date"]', generateDate());
    await page.locator('select[name="lead_type_id"]').selectOption('1');
    await page.locator('select[name="user_id"]').selectOption('1');
    await page.fill('input[name="lead_value"]', '1000');

    /**
     * Add a new contact person via the lookup component.
     */
    const contactSection = page.locator('#contact-person');
    ...
    await page.fill('input[name="person[emails][0][value]"]', leadEmail);
    await page.fill('input[name="person[contact_numbers][0][value]"]', leadPhoneNumber);

    /**
     * Remove the auto-added empty product row to avoid its required-product
     * validation error on save.
     */
    while (await page.locator('#products .icon-delete').count() > 0) {
        await page.locator('#products .icon-delete').first().click();
    }

    await page.getByRole('button', { name: 'Save' }).click();
    await page.waitForURL(/\/admin\/leads(?:\?.*)?$/);

    return { leadTitle, leadEmail };
}

/**
 * Open a Lead's view page by searching the grid for its title.
 *
 * @param {import('@playwright/test').Page} page  An authenticated admin page.
 * @param {string} leadTitle
 */
async function openLeadByTitle(page, leadTitle) {
    await page.goto('admin/leads');
