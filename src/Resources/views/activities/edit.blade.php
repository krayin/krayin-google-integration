    ...
    <script type="text/javascript">
        $(function() {
            ...
            if (account && account.scopes.includes('meet')) {
                ...
                if (activity.location && activity.location.includes('meet.google.com')) {
                    $('.video-conference').append('<button type="button" class="btn btn-sm btn-secondary-outline create-link" id="create-google-meet-link" style="display: none"><i class="icon google-meet-icon"></i>Google Meet</button>');

                    $('.video-conference').append('<span class="join-google-meet-link join-link"><a href="' + activity.location + '" target="_blank" class="btn btn-sm btn-secondary-outline">Join Google Meet</a><i class="icon trash-icon" id="remove-google-meet-button"></i></span>');
                } else {
